...
const Circulation = ({ subsidy }) => {
    ...
    const EDF = 4732628;
    // Future Development Fund', value: 4204800
    const FDF = 4204800;
    ...
    //             // console.log('地址余额:', decimalEDFBalance, balance);
    //             const EDFtoWei = web3.utils.toWei(EDF,'ether');
    //             // console.log('EDFtoWei:', EDFtoWei);
    //             // const bigEDFLiquid = EDFtoWei - parseFloat(decimalEDFBalance);
    //             const bigEDFLiquid = web3.utils.toBigInt(EDFtoWei) - web3.utils.toBigInt(balance);
    //             const decimalEDFLiquid = web3.utils.fromWei(bigEDFLiquid,'ether')
    //             console.log("生态基金流通:",decimalEDFLiquid)
    //             setEDFLiquid(parseFloat(decimalEDFLiquid));
    //         })
    //         .catch(error => {
    ...
    const totalCirculation = GEM + FTI + SRW + subsidy + EDF + FDF;
    ...
    <div>
        ...
        <p>
            Therefore, the main components of current circulation is: Genesis Mapping {GEM} Meer(20%), Founding Team Incentives {FTI} Meer(2%), Umayyad 1.0 Staking Reward {SRW} Meer(3.53%), Current PoW Total Subsidy {subsidy} Meer({subsidyRatio})，the Eco-development Fund {EDF} Meer(2.25%), the Future Development Fund {FDF} Meer(2%). Also note that, the Eco-development Fund and the Future Development Fund will be gradually and irregularly released into the circulation market according to their use.
        </p>
        <p>
